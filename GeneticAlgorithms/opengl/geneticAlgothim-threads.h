...
// global vars
inline int size_tournament = 3; // 2;
inline float prob_mut = 0.05;
inline int contPob = 0;
...
inline std::vector<individual> initialize(int tam) {
...
template<size_t N>
inline void printCross(std::bitset<N>& p1, std::bitset<N>& p2, std::bitset<N>& h1, std::bitset<N>& h2, char c, int cut) {
...
inline void crossover(individual& p1, individual& p2, individual& h1, individual& h2, std::mt19937& local_rng) {
    h1.x = p1.x; h1.y = p1.y;
    h2.x = p2.x; h2.y = p2.y;

    // CROOSOVER -> X
    int cutX = randInt(1, 4, local_rng);
    ...
    // CROOSOVER -> Y
    int cutY = randInt(1, 5, local_rng);
...
template<size_t N>
inline void printMut(std::bitset<N>& h, char c, int mut, int n) {
...
inline void mutation(individual& in, int n) {
    // MUTATION -> X
    for (int i = 0; i < 5; i++) {
...
inline void printBest(const individual& in, int generacion) {
...
inline void printPob(const std::vector<individual>& pob) {
...
inline void evaluate(std::vector<individual>& pob, std::vector<individual>& newPob, int i, int sizePob) {
    ...
    // crossover
    crossover(p1, p2, h1, h2, local_rng);
