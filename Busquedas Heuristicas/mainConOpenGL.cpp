...
// -- hillClimbing --
struct compHill {
  bool operator()(const std::pair<coord, double>& a,
				  const std::pair<coord, double>& b) {
...
std::vector<coord> hillClimbing(std::map<coord, std::vector<coord>>& graph, int x, int y, int fin1, int fin2) {
	...
	while (!nose.empty()) {
		...
		nose.pop();
		if (tempCoord == fin) { found = true; break; }
...
void changeScreen(unsigned char key, int, int) {
	...
	if (key == 'r' || key == 'R') {
		...
		// meter todos los nodos en un vector
		std::vector<coord> nodos;
		for (auto& kv : graph)
			nodos.push_back(kv.first);
		
		int toRemove = nodos.size() * 0.2f; // calculate -> delete
		...
		for (int i = 0; i < toRemove; i++) {
			...
			// delete -> list adjacency
			for (auto& nb : graph[nodo]) {
				auto& vec = graph[nb];
				vec.erase(std::remove(vec.begin(), vec.end(), nodo), vec.end());
			}
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Búsquedas - Heuristicas");
