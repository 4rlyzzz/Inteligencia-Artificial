...
// colors and results
inline std::vector<std::string> color = { "red", "green", "blue" };
inline std::map<coord, std::vector<std::string>> domColor; // for "forwardChecking" and "moreRestricted"
inline std::map<coord, std::string> resForward, resRestrictive, resRestricted; // results
...
inline void removeNodesAndConnections(int num) {
    // REMOVE NODES
        ...
        if (graph.count(tempN)) {
            for (auto& t : graph[tempN]) {
                ...
                if (graph.count(t)) {
                    ...
                    neigh.erase(std::remove(neigh.begin(), neigh.end(), tempN), neigh.end()); // delete 
    ...
    for (auto& neighGraph : graph) {
        ...
        if (neighGraph.second.size() < 2) { // add neigh
            int target = randInt(2, 5);

            while (neighGraph.second.size() < target) {

                int r = randInt(0, nodes.size() - 1);
            ...
            while (neighGraph.second.size() > 5) {
                int r = randInt(0, neighGraph.second.size() - 1);
...
inline void deleteColor(std::map<coord, vString>& domActual, coord nodo, std::string colorAssign) {
...
// for "forwardChecking" and "moreRestricted"
inline void resetDom() {
    domColor.clear();
    for (auto& i : graph)
...
// -- forwardChecking --
inline bool forwardChecking(std::map<coord, vString> domActual, vCoord nodosRemaining,
    int indxNode, std::map<coord, std::string>& assignmentFinal) {

    if (indxNode == nodosRemaining.size())
        return true;
    ...
    for (auto& colorTest : domActual[nodoActual]) {
        ...
        for (const auto& i : domActual[nodoActual])
            std::cout << i << " ";
        std::cout << "}\n";
        // END DEBUG
...
inline void callForwardChecking() {
    resetDom();
...
// -- moreRestrictive --
inline bool moreRestrictive(vCoord nodosRemaining, int indxNode, std::map<coord, std::string>& assignmentFinal) {

    if (indxNode == nodosRemaining.size()) return true;
...
inline void callMoreRestrictive() {
...
// -- moreRestricted --
inline bool moreRestricted(std::map<coord, vString> domActual, std::map<coord, std::string>& assignmentFinal) {
